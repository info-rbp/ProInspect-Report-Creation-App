...
export const SECURITY_CAPABILITIES = [
  ...
  'client.read', 'client.manage', 'client.contact.manage', 'client.relationship.manage', 'client.engagement.manage', 'client.document.manage', 'client.portal.manage', 'client.commercial.manage',
...
const settingsAdministration = [
  'settings.read', 'settings.organisation.manage', 'settings.branding.manage', 'settings.operations.manage', 'settings.communications.manage', 'settings.security.manage',
  'integration.read', 'integration.manage', 'integration.credentials.manage', 'integration.sync', 'maintenance.policy.manage',
...
const tenantWorkspace = [
  'tenant.read', 'tenant.manage', 'tenancy.read', 'tenancy.manage', 'tenant.communication.read', 'tenant.communication.send', 'tenant.document.read', 'tenant.document.manage', 'tenant.portal.manage', 'tenant_instruction.manage',
] satisfies SecurityCapability[];
...
const operationalEnhancements = [
  'job.plan', 'job.plan.publish', 'job.offline.sync', 'job.remote.manage',
  'communication.read', 'communication.send', 'communication.manage',
  'compliance.read', 'compliance.manage', 'compliance.rule.manage',
...
export const ROLE_CAPABILITIES: Readonly<Record<UserRole, readonly SecurityCapability[]>> = {
  ...
  proinspect_admin: [
    ...
    'user.read', 'user.invite', 'user.profile.manage', 'user.role.manage', 'user.scope.manage', 'user.suspend', 'user.reactivate', 'user.revoke', 'user.session.revoke', 'user.security.manage', 'user.audit.read',
    'workforce.read', 'workforce.manage', ...clientAdministration,
    'property.read', 'property.manage', ...tenantWorkspace, ...documentOperations,
    'job.read', 'job.manage', 'job.inspect', ...operationalEnhancements,
    'report.read', 'report.edit', 'report.review', 'report.issue', 'report.finalise',
    'template.manage', 'audit.read',
    'maintenance.manage', 'maintenance.read', 'maintenance.triage', 'maintenance.quote.prepare', 'maintenance.quote.approve', 'maintenance.quote.send', 'maintenance.work_order.issue', 'maintenance.verify', 'price_book.manage',
    'upload.create', 'analysis.create', 'pdf.create', 'notification.send', 'external_contact.manage', 'client_approval.manage',
  ],
  operations: [
    'agency.read', 'settings.read', 'integration.read', 'integration.sync', 'user.read', 'workforce.read', ...clientAdministration,
    'property.read', 'property.manage', ...tenantWorkspace,
    'document.policy.read', 'document.packet.read', 'document.packet.manage', 'document.packet.issue', 'document.sign.manage', 'notice.issue', 'service_record.manage',
    'job.read', 'job.manage', 'job.plan', 'job.plan.publish', 'job.offline.sync', 'job.remote.manage',
    'report.read', 'report.issue',
    'maintenance.manage', 'maintenance.read', 'maintenance.triage', 'maintenance.quote.prepare', 'maintenance.quote.send', 'maintenance.work_order.issue', 'maintenance.verify',
    'communication.read', 'communication.send', 'communication.manage', 'compliance.read', 'compliance.manage', 'key.read', 'key.manage', 'analytics.read',
    'upload.create', 'analysis.create', 'pdf.create', 'notification.send', 'external_contact.manage', 'client_approval.manage',
  ],
  inspector: [
    'property.read', 'tenant.read', 'tenancy.read', 'job.read', 'job.inspect', 'job.offline.sync', 'report.read', 'report.edit', 'upload.create', 'maintenance.read', 'maintenance.triage', 'key.read', 'key.manage',
  ],
  analyst: [
    'property.read', 'tenant.read', 'tenancy.read', 'job.read', 'report.read', 'report.edit', 'maintenance.read', 'maintenance.triage', 'maintenance.quote.prepare', 'analysis.create',
    'tenant_instruction.manage', 'tenant.communication.read', 'tenant.document.read', 'document.packet.read', 'communication.read', 'compliance.read', 'external_contact.manage', 'client_approval.manage',
  ],
  reviewer: [
    'property.read', 'tenant.read', 'tenancy.read', 'job.read', 'report.read', 'report.review', 'audit.read', 'pdf.create', 'maintenance.read', 'maintenance.triage', 'maintenance.verify',
    'tenant_instruction.manage', 'tenant.communication.read', 'tenant.document.read', 'document.packet.read', 'document.packet.approve', 'communication.read', 'compliance.read', 'client_approval.manage',
  ],
...
export const INTERNAL_SECTION_CAPABILITIES: Readonly<Record<InternalSection, readonly SecurityCapability[]>> = {
  dashboard: [], clients: ['client.read'], properties: ['property.read'], jobs: ['job.read'], reports: ['report.read'], maintenance: ['maintenance.read'], tenants: ['tenant.read'],
  communications: ['communication.read'], compliance: ['compliance.read'], users: ['user.profile.manage'], templates: ['template.manage'], settings: ['settings.read'],
};
...
export function canAccessInternalSection(role: UserRole | undefined, section: InternalSection): boolean { if (!role || !isInternalRole(role)) return false; return INTERNAL_SECTION_CAPABILITIES[section].every((capability) => roleHasCapability(role, capability)); }

export interface AuthenticatedPrincipal { uid: string; email?: string; tenantId?: string; agencyId: string; role: UserRole; mfaVerified: boolean; sessionId?: string; tokenIssuedAt: number; }
export interface AgencyMembership { uid: string; agencyId: string; role: UserRole; status: 'invited' | 'active' | 'suspended' | 'revoked'; invitationExpiresAt?: string; mfaRequired: boolean; propertyIds?: string[]; tenancyIds?: string[]; inspectionJobIds?: string[]; reportIds?: string[]; clientAccountIds?: string[]; updatedAt: string; }
