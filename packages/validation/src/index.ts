...
export interface ValidationSchema<T> { parse(value: unknown): ValidationResult<T>; }
export interface ResourceWriteInput { id?: string; agencyId?: string; version?: number; [key: string]: unknown; }
export type WorkflowStatus = InspectionJobStatus | ReportLifecycleStatus;
export interface WorkflowTransitionInput { status: WorkflowStatus; expectedVersion: number; reason?: string; assignedUserId?: string; }
export type UploadSessionInput = EvidenceUploadSessionInput;
export interface TaskCreationInput { reportId: string; reportVersionId?: string; priority?: 'normal' | 'high'; }
export interface TenantResponseInput { reportId: string; tenancyId: string; responses: unknown[]; expectedVersion: number; }
...
export const resourceWriteSchema: ValidationSchema<ResourceWriteInput> = {
  parse(value) {
    ...
    const forbidden = ['createdAt','updatedAt','actorId','actorRole','finalPdfUrl','finalPdfReportVersionId','finalPdfObjectPath','finalPdfSha256','finalPdfGeneration','renderManifestObjectPath','renderManifestSha256','pdfGeneratedAt','archiveReportVersionId','archiveManifestObjectPath','archiveManifestSha256','archiveCreatedAt'];
...
export const workflowTransitionSchema: ValidationSchema<WorkflowTransitionInput> = {
  parse(value) {
    const parsed = record(value); if (!parsed.ok) return parsed;
    const status = parsed.value.status; const expectedVersion = parsed.value.expectedVersion;
    ...
    const version = requirePositiveInteger(expectedVersion, 'expectedVersion'); if (!version.ok) return version;
    const reason = parsed.value.reason; if (reason !== undefined && typeof reason !== 'string') return validationError('reason must be a string.', { field: 'reason' });
    const assignedUserId = parsed.value.assignedUserId; if (assignedUserId !== undefined && typeof assignedUserId !== 'string') return validationError('assignedUserId must be a string.', { field: 'assignedUserId' });
    return { ok: true, value: { status: status as WorkflowStatus, expectedVersion: version.value, ...(reason ? { reason: reason.trim() } : {}), ...(assignedUserId ? { assignedUserId: assignedUserId.trim() } : {}) } };
  },
};
