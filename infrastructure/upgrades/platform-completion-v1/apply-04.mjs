...
function collectionReadRecord(
  ...
  if (
    ...
    && typeof value.evidencePhotoIds === 'string'
  ) {
    try {
      value.evidencePhotoIds = JSON.parse(value.evidencePhotoIds) as unknown[];
    } catch {
      value.evidencePhotoIds = [];
    }
  }
  return value;
}
...
function automationSecretMatches(req: IncomingMessage): boolean {
  const supplied = req.headers['x-proinspect-automation-secret']
    ?.toString();
...
async function automationGrant(
  ...
  if (!automationSecretMatches(req)) {
    throw new ApiError(
      ...
      'Automation grant issuance is not authorised.',
    );
  }
  ...
  const created = await createGrant(
    ...
    recipientEmail,
    24 * 7,
    'system:tenant-automation',
  );
  await dependencies.audit.append({
    id: randomUUID(),
    timestamp: new Date().toISOString(),
    actorId: 'system:tenant-automation',
    actorRole: 'operations',
    agencyId,
    capability: 'tenant.portal.manage',
    outcome: 'allowed',
    reason: 'tenant_portal.automation_grant_generated',
    target: { agencyId, tenancyId },
    correlationId,
  });
  return {
    status: 201,
...
const workerPortalLink = `async function portalLink(agencyId: string, tenantId: string, tenancyId: string, recipientEmail: string): Promise<string> {
  ...
  if (!apiBase || !secret) {
    throw new Error('PROINSPECT_API_BASE_URL and AUTOMATION_RUNNER_SECRET are required for tenant portal automation.');
  }
  const response = await fetch(\`${'${apiBase}'}/api/v1/internal/tenant-portal-grants/automation\`, {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
      'x-proinspect-automation-secret': secret,
    },
  ...
  if (!response.ok || !payload.data?.accessUrl) {
    throw new Error(payload.error?.message || \`Tenant portal grant API failed with \${response.status}.\`);
  }
...
export async function applyStage04() {
  replaceOnce(
    ...
    "  clients: 'clients',\n  clientContacts: 'client_contacts', clientApprovals: 'client_approvals',",
    'clientApprovals Appwrite mapping',
  );
  insertBefore(
    'apps/api/src/backend/appwriteAdapters.ts',
    'export function createAppwriteApiServices(',
    collectionCodec,
    'collection codec helpers',
  );
  replaceOnce(
    'apps/api/src/backend/appwriteAdapters.ts',
    ...
    'collection-aware list decoding',
  );
  replaceOnce(
    'apps/api/src/backend/appwriteAdapters.ts',
  ...
  replaceOnce(
    ...
    'collection-aware create encoding',
  );
  replaceOnce(
    'apps/api/src/backend/appwriteAdapters.ts',
    ...
    'collection-aware create decoding',
  );
  replaceOnce(
    'apps/api/src/backend/appwriteAdapters.ts',
    '    const patch = writeData(data, actorId, current);',
    '    const patch = writeData(collectionWriteData(collection, data), actorId, current);',
    'collection-aware update encoding',
  );
  replaceOnce(
    'apps/api/src/backend/appwriteAdapters.ts',
    '    return publicRecord(row as unknown as Record<string, unknown>);\n  }\n}\n\nfunction appwriteErrorCode',
    '    return collectionReadRecord(collection, row as unknown as Record<string, unknown>);\n  }\n}\n\nfunction appwriteErrorCode',
    'collection-aware update decoding',
  );

  insertAfter(
    'infrastructure/appwrite/tables/schema.mjs',
    "  agencyEntity('maintenance_approvals', [str('maintenanceItemId',36,true),str('quoteId',36),str('approvedBy',36),str('decision',32,true),text('reason'),datetime('decidedAt')], [index('item_decision',['maintenanceItemId','decision'])]),\n",
    clientApprovalTable,
  ...
  replaceOnce(
    ...
    'client approvals schema contract',
  );
  replaceOnce(
    'infrastructure/appwrite/tables/schema.test.mjs',
    'expect(allTables).toHaveLength(119);',
    'expect(allTables).toHaveLength(120);',
    '120-table Stage 4 baseline',
  );
  ...
  replaceOnce(
    ...
    'tenant automation timing-safe import',
  );
  insertBefore(
    'apps/api/src/backend/tenantPortalRoutes.ts',
    'async function generateGrant(',
    automationRoute,
    'internal automation grant route',
  );
  insertAfter(
    'apps/api/src/backend/tenantPortalRoutes.ts',
  ...
  for (const path of [
    ...
    replaceRegexOnce(
      path,
      /async function portalLink\(agencyId: string, tenantId: string, tenancyId: string, recipientEmail: string\): Promise<string> \{[\s\S]*?\n\}/u,
      workerPortalLink,
      `${path} canonical tenant grant API`,
    );
  }
}
