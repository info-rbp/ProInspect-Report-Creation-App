...
check(manifest.development?.appwriteProjectId === 'proinspect-development', 'Development Appwrite target is pinned');
check(manifest.prohibitedProjectIds?.includes('6a911f1e0031e90015b2'), 'legacy Appwrite project is prohibited');
...
for (const marker of ["./apply-02e-v2.mjs","./apply-04-v2.mjs","./apply-07.mjs","stage2eProviderBoundary.test.ts","stage4RuntimeParity.test.ts","stage7OfflineContract.test.ts"]) check(apply.includes(marker), `apply sequence contains ${marker}`);
...
for (const script of ['upgrade:status','upgrade:preflight','upgrade:apply','upgrade:verify','upgrade:audit','upgrade:install']) check(Boolean(rootPackage.scripts?.[script]), `root package exposes ${script}`);
