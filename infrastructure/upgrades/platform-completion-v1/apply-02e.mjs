...
export async function applyStage02e() {
  writeIfChanged(
    ...
    payload('appwriteEvidenceStore.ts'),
  );
  writeIfChanged(
    'apps/api/src/backend/firestoreEvidenceStore.ts',
    payload('firestoreEvidenceStore.ts'),
  );
  writeIfChanged(
    'apps/api/src/backend/externalEvidenceCompletionRoutes.ts',
    payload('externalEvidenceCompletionRoutes.ts'),
  );
  ...
  replaceOnce(
    ...
    'ApiDependencies evidence store',
  );
  ...
  replaceOnce(
    ...
    '  ApiDependencies,\n  ExternalGrantStore,',
    '  ApiDependencies,\n  EvidenceStore,\n  ExternalGrantStore,',
    'EvidenceStore dependency type import',
  );
  replaceOnce(
    'apps/api/src/security/defaultDependencies.ts',
    ...
    'default Firestore evidence provider',
  );
  replaceOnce(
    'apps/api/src/security/defaultDependencies.ts',
    '    externalGrants = new AppwriteExternalGrantStore(appwrite);\n    idempotency =',
    '    externalGrants = new AppwriteExternalGrantStore(appwrite);\n    evidence = new AppwriteEvidenceStore(appwrite);\n    idempotency =',
    'Appwrite evidence provider wiring',
  );
  replaceOnce(
    'apps/api/src/security/defaultDependencies.ts',
    '    externalGrants,\n    idempotency,',
    '    externalGrants,\n    evidence,\n    idempotency,',
    'returned evidence provider',
  );

  replaceOnce(
    'packages/domain/src/photoEvidence.ts',
    "externalResourceType?: 'work_request' | 'tenant_instruction' | 'tenant_portal';",
    "externalResourceType?: 'work_request' | 'tenant_instruction' | 'report_distribution' | 'tenant_portal' | 'remote_inspection';",
    'UploadSessionRecord resource type parity',
  );

  replaceOnce(
    'apps/api/src/backend/integrations.ts',
    "input.externalResourceType === 'work_request' || input.externalResourceType === 'tenant_instruction' || input.externalResourceType === 'tenant_portal'",
    "input.externalResourceType === 'work_request' || input.externalResourceType === 'tenant_instruction' || input.externalResourceType === 'report_distribution' || input.externalResourceType === 'tenant_portal' || input.externalResourceType === 'remote_inspection'",
    'Firebase fallback resource type parity',
  );

  replaceOnce(
    'infrastructure/appwrite/tables/schema.mjs',
    "str('legacyFileId',255)]",
    "str('legacyFileId',255),str('storageVersion',128)]",
    'evidence storage version column',
  );

  replaceOnce(
    'apps/api/src/backend/externalEvidenceRoutes.ts',
    '  return {\n    status: 201,\n    body: { data: { ...session, photoId: uploadId }, meta: { correlationId } },\n  };',
    `  const encodedToken = encodeURIComponent(decodeURIComponent(parts[4]));\n  const providerUrls = session.uploadProvider === 'appwrite'\n    ? {\n        binaryUploadUrl: \`/api/v1/external/evidence/\${encodedToken}/upload-session/\${encodeURIComponent(uploadId)}/binary\`,\n        completionUrl: \`/api/v1/external/evidence/\${encodedToken}/upload-session/\${encodeURIComponent(uploadId)}/complete\`,\n      }\n    : {};\n  return {\n    status: 201,\n    body: { data: { ...session, ...providerUrls, photoId: uploadId }, meta: { correlationId } },\n  };`,
    'external evidence provider upload URLs',
  );
...
function patchTenantPortal() {
  replaceOnce(
    ...
    '  type MaintenancePriority,\n} from \'@pcr/domain\';\nimport { authenticateAndAuthorise } from \'../security/authoriseRequest.js\';',
    'tenant evidence legacy persistence imports',
  );
  replaceOnce(
    'apps/api/src/backend/tenantPortalRoutes.ts',
    ...
    'tenant evidence provider import',
  );
  replaceOnce(
    'apps/api/src/backend/tenantPortalRoutes.ts',
  ...
  replaceOnce(
    ...
    'tenant upload session route',
  );

  replaceOnce(
    'apps/web/services/platform/tenantPortalService.ts',
    'const session = await externalRequest<{ id: string; status: string; resumableUploadUrl?: string; duplicatePhotoId?: string }>',
    'const session = await externalRequest<{ id: string; status: string; resumableUploadUrl?: string; binaryUploadUrl?: string; completionUrl?: string; duplicatePhotoId?: string }>',
    'tenant Appwrite upload fields',
  );
  replaceRegexOnce(
    'apps/web/services/platform/tenantPortalService.ts',
    /  if \(!session\.resumableUploadUrl\) throw new Error\('Evidence upload service is not configured\.'\);\n  const upload = await fetch\(session\.resumableUploadUrl,[\s\S]*?  return externalRequest\(`\/api\/v1\/external\/tenant-portal\/\$\{encodeURIComponent\(grantToken\)\}\/evidence\/upload-session\/\$\{encodeURIComponent\(session\.id\)\}\/complete`, \{ method: 'POST', headers: \{ 'content-type': 'application\/json' \}, body: '\{\}' \}\);/u,
...
function patchRemoteInspection() {
  ...
  replaceRegexOnce(
    ...
    'remote evidence upload transport',
  );
}
...
function patchEmulatorHarness() {
  replaceOnce(
    ...
    'emulator evidence store import',
  );
  replaceOnce(
    'tests/emulator/externalEvidence.emulator.test.ts',
    '    externalGrants:\n      new FirestoreExternalGrantStore(),\n    reports:',
    '    externalGrants:\n      new FirestoreExternalGrantStore(),\n    evidence: new FirestoreEvidenceStore(),\n    reports:',
    'emulator evidence dependency',
  );
}
