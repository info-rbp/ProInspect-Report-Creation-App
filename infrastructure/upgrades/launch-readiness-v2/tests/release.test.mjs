...
function configured(){
  ...
  target.cloudflare.accountId='0123456789abcdef0123456789abcdef';target.cloudflare.workerName='proinspect-production-edge';target.cloudflare.apiOrigin='https://api-prod.example.run.app';
...
test('Production Shopify webhook is pinned to the Cloudflare public edge',()=>{const config=configured();config.environments.production.shopify.webhookUri='https://api-prod.example.run.app/webhook';assert.throws(()=>validateProductionConfig(config),/Cloudflare public API edge/);});
