...
import { credentialMatchesPolicy, runtimeCredentialBootstrap } from '../actions/credentials.mjs';
...
test('Appwrite capability detection remains fail-closed for unsupported stored-key creation',async()=>{
...
test('runtime credential bootstrap derives non-secret operator handoff variables and enforces policy metadata',()=>{
  const env={APPWRITE_RUNTIME_KEY_ID_PDF_WORKER:'key-123',APPWRITE_RUNTIME_SECRET_VERSION_PDF_WORKER:'7'};
  assert.deepEqual(runtimeCredentialBootstrap('pdf-worker',env),{keyId:'key-123',secretVersion:'7',keyIdName:'APPWRITE_RUNTIME_KEY_ID_PDF_WORKER',versionName:'APPWRITE_RUNTIME_SECRET_VERSION_PDF_WORKER'});
  assert.throws(()=>runtimeCredentialBootstrap('pdf-worker',{}),/APPWRITE_RUNTIME_KEY_ID_PDF_WORKER/);
  const policy={service:'pdf-worker',expiryHours:24,scopes:['rows.read','files.read']};
  const key={$id:'key-123',name:'launch-development-pdf-worker-20260913',scopes:policy.scopes,expire:new Date(Date.now()+23*3600000).toISOString()};
  assert.equal(credentialMatchesPolicy(key,policy,'development'),true);
  assert.equal(credentialMatchesPolicy({...key,name:'unrelated'},policy,'development'),false);
  assert.equal(credentialMatchesPolicy({...key,scopes:['rows.write']},policy,'development'),false);
});
test('Appwrite retains target validation, raw inventory and operator-assisted guarded credential mutation',()=>{
  ...
  assert(session.includes('APPWRITE_INSTALLER_KEY_ID'));assert(!session.includes("['project','create-key'"));assert(!session.includes('create-ephemeral-key'));
  const credentials=readFileSync(resolve(packageDir,'actions/credentials.mjs'),'utf8');
  assert(credentials.includes('APPWRITE_RUNTIME_KEY_ID_'));assert(credentials.includes('APPWRITE_RUNTIME_SECRET_VERSION_'));
  assert(!credentials.includes("['project','create-key'"));assert(!credentials.includes('requireRuntimeKeyCreation'));
  assert(credentials.indexOf('credentialMatchesPolicy')<credentials.indexOf("await cli(['project','delete-key'"));
});
