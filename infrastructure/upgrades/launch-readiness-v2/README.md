...
CLI 27.2.1 uses `project` for project identity, platform and key inventory. Read responses use `--raw` to preserve empty arrays and null values. Revocable temporary credentials require stored-key creation with repeated `--scopes` flags, exact scope/expiry validation and mandatory deletion. CLI 27.2.1 cannot create these keys. Its `create-ephemeral-key` command issues a stateless JWT and is not an equivalent revocable credential; the temporary-key path fails before mutation instead of substituting it. Web platforms use `project create-web-platform` with an explicit platform ID. The installer still refuses a standing `APPWRITE_API_KEY` and verifies the exact target before requesting temporary credentials.
