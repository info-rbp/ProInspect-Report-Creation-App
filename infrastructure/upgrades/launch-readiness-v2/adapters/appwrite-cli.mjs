...
export async function appwriteSchemaAudit(target,directory){
  ...
  const keys=await list(['projects','list-keys','--project-id',app.projectId,'--limit','100'],'keys');const platforms=await list(['projects','list-platforms','--project-id',app.projectId,'--limit','100'],'platforms');const domainMatches=platforms.filter((p)=>p.type==='web'&&p.hostname===new URL(target.web.origin).hostname);
