...
import { root, run, readJson, hash, requireThat, atomicJson } from '../runtime.mjs';
...
export async function terraform(target,context,directory) {
  ...
  const digest=hash(readFileSync(planFile)); atomicJson(resolve(directory,'terraform-approval.json'),{candidate:context,planSha256:digest});
  await googleIdentity(target.google); requireThat(hash(readFileSync(planFile))===digest,'Terraform plan changed');
  await run('terraform',['apply','-input=false',planFile],{cwd,live:true,sensitive:true,timeoutMs:3600000});
  return {planSha256:digest,applied:true,resources:plan.resource_changes?.length ?? 0};
}
