...
export async function sourceSchema() {
  ...
  const c = await import(pathToFileURL(resolve(directory,'tables/worker-runtime-extensions.mjs')).href);
  return { databases: readJson(resolve(directory,'databases/databases.json')), tables: [...a.tables,...b.unifiedPlatformExtensionTables,...c.workerRuntimeExtensionTables], buckets: readJson(resolve(directory,'buckets/buckets.json')), teams: readJson(resolve(directory,'teams/teams.json')) };
}
