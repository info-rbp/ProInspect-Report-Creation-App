...
- Appwrite Auth browser/provider wiring for session restore, email/password login and logout, email verification, password recovery, TOTP enrollment/challenge, recovery codes, disabled accounts, and authoritative API membership resolution.
...
Launch-blocking work includes dedicated transactional commands for high-value transitions, production feature screens behind the portal shell, the shared offline queue/conflict experience, Development-only migration and binary-copy runners, Shopify generic routing/replay tests, worker Appwrite adapters, authenticated seven-portal E2E, file-recovery rehearsal and real device testing. The database backup/restore mechanism itself was rehearsed successfully in Development on 2026-09-02.
...
The Strata deployment and repository remain available for rollback. Archive is prohibited until every parity row has a final status of `COMPLETE`, `RETIRED_WITH_APPROVAL`, `EXTERNAL_BY_DESIGN`, or a genuine `BLOCKED_OWNER_INPUT`; D1/R2 reconciliation and idempotent reruns pass; all cut-over worker paths use the canonical API/Appwrite boundary; and rollback has been rehearsed.
