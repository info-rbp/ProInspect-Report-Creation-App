...
const allowedStatuses = new Set(['COMPLETE', 'RETIRED_APPROVED', 'BLOCKED_OWNER_INPUT']);
...
if (!process.exitCode) {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (entry.status === 'BLOCKED_OWNER_INPUT'
      && (!Array.isArray(entry.ownerActions) || entry.ownerActions.length === 0)) {
      fail(`${entry.capability} is BLOCKED_OWNER_INPUT but has no ownerActions.`);
    }
    if (entry.status === 'RETIRED_APPROVED'
      && (typeof entry.approvalReference !== 'string' || !entry.approvalReference.trim())) {
      fail(`${entry.capability} is RETIRED_APPROVED but has no approvalReference.`);
    }
  }
