...
class MemoryReportStore implements ReportAggregateStore {
  ...
  private key(agencyId: string, reportId: string) {
  ...
  async saveDraft(aggregate: ReportAggregate, expectedVersion?: number) {
    ...
    if (previous?.report.version && expectedVersion !== previous.report.version) {
      throw Object.assign(new Error('Version conflict.'), {
        status: 409,
        code: 'VERSION_CONFLICT',
      });
    ...
    const stored: ReportAggregate = {
      ...
      report: {
        ...
        createdAt: previous?.report.createdAt ?? now,
        updatedAt: now,
      },
    };
...
async function routeRequest(
  ...
  const response = await fetch(`http://127.0.0.1:${address.port}${path}`, {
    ...
    headers: {
      ...
      'idempotency-key': 'specialised-test-key',
    },
...
function writeResponse(res: ServerResponse, response: { status: number; body: unknown }) {
...
function legacyReport(): ReportAggregate {
  return {
    report: {
      ...
      tenancyId: 'tenancy-1',
      inspectionJobId: 'exit-job-1',
      reportType: 'Exit Inspection',
      propertyAddress: '1 Test Street',
      lifecycleStatus: 'draft',
      templateId: 'system-exit-v1',
      templateVersion: 1,
      baselineReportId: 'legacy:legacy-exit-report-1',
      baselineReportVersionId: 'legacy-exit-report-1',
        ...
        components: [
          {
            ...
            commentary: 'Current Exit observation.',
            photoReferences: [],
            reviewStatus: 'draft',
...
describe('reviewed legacy baseline mapping', () => {
  it('binds reviewed legacy facts to the stable Exit component and keeps comparison review required', async () => {
    ...
    const response = await routeRequest(
      async (req, res) => {
        try {
          writeResponse(res, (await routeLegacyBaselineRequest(req, deps, 'corr-1'))!);
        } catch (error) {
          const candidate = error as { status?: number; code?: string; message?: string };
          writeResponse(res, { status: candidate.status ?? 500, body: { error: candidate } });
        }
      },
      '/api/v1/reports/exit-report-1/legacy-baseline',
      {
        expectedVersion: 1,
        source: {
          sourceId: 'legacy-entry-2025',
          sourceType: 'pdf',
          sourceName: 'Entry PCR 2025.pdf',
        },
        mappings: [
          {
            areaId: 'bathroom',
            componentId: 'mirror',
            sourceAreaLabel: 'Bathroom',
            sourceComponentLabel: 'Mirror',
            sourceExcerpt: 'Mirror clean and intact.',
            baseline: {
              conditionCategory: 'intact',
              cleanlinessCategory: 'clean',
              workingStatus: 'not_applicable',
              testStatus: 'not_applicable',
              commentary: 'Mirror clean and intact.',
              defects: [],
              photoReferences: [],
            },
            mappingMethod: 'legacy_mapping',
            confidence: 0.6,
            reviewNote: 'Direct label match reviewed against Entry PDF.',
          },
        ],
      },
    );
    ...
    expect(
      await repository.get('legacyBaselineMappings', 'agency-a', 'legacy-exit-report-1'),
    ).toBeTruthy();
  });
  ...
  it('rejects a legacy operational confirmation without a passed test result', async () => {
    ...
    const response = await routeRequest(
      async (req, res) => {
        try {
          writeResponse(res, (await routeLegacyBaselineRequest(req, deps, 'corr-2'))!);
        } catch (error) {
          const candidate = error as { status?: number; code?: string; message?: string };
          writeResponse(res, { status: candidate.status ?? 500, body: { error: candidate } });
        }
    ...
    expect(await response.json()).toMatchObject({
      error: { code: 'LEGACY_OPERATION_UNSUPPORTED' },
    });
  });
});
...
describe('targeted maintenance follow-up report creation', () => {
  it('creates only the selected maintenance target and resets the current assessment conservatively', async () => {
    ...
    await repository.create(
      ...
      'agency-a',
      'property-1',
      { address: '1 Test Street', status: 'active' },
      'admin-1',
    );
    await repository.create(
      'maintenanceItems',
      ...
      'maintenance-1',
      {
        ...
        description: 'Fan operation requires follow-up.',
        category: 'Electrical',
        priority: 'routine',
        status: 'verified',
        sourceEvidenceIds: ['photo-before'],
    ...
    const response = await routeRequest(
      async (req, res) => {
        try {
          writeResponse(res, (await routeMaintenanceReportRequest(req, deps, 'corr-3'))!);
        } catch (error) {
          const candidate = error as { status?: number; code?: string; message?: string };
          writeResponse(res, { status: candidate.status ?? 500, body: { error: candidate } });
        }
      },
      '/api/v1/maintenance-reports/create',
      { maintenanceItemIds: ['maintenance-1'], inspectionDate: '2026-08-17' },
    );
    expect(response.status).toBe(201);
    const payload = (await response.json()) as { data: ReportAggregate };
    ...
    expect(
      await repository.get(
        'inspectionJobs',
        'agency-a',
        String(payload.data.report.inspectionJobId),
      ),
    ).toBeTruthy();
  });
  ...
  it('rejects mixed-property maintenance items', async () => {
    ...
    const response = await routeRequest(
      async (req, res) => {
        try {
          writeResponse(res, (await routeMaintenanceReportRequest(req, deps, 'corr-4'))!);
        } catch (error) {
          const candidate = error as { status?: number; code?: string; message?: string };
          writeResponse(res, { status: candidate.status ?? 500, body: { error: candidate } });
        }
      ...
      '/api/v1/maintenance-reports/create',
      { maintenanceItemIds: ['one', 'two'] },
    );
    expect(response.status).toBe(409);
    expect(await response.json()).toMatchObject({
      error: { code: 'MAINTENANCE_CONTEXT_MISMATCH' },
    });
  });
});
