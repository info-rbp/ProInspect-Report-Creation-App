...
class MemoryReports implements ReportAggregateStore {
  ...
  async load(agencyId: string, reportId: string) {
    return this.values.get(`${agencyId}:${reportId}`);
  }

  async saveDraft(aggregate: ReportAggregate) {
    const stored = { ...aggregate, report: { ...aggregate.report, version: 1 } };
    this.values.set(`${aggregate.report.agencyId}:${aggregate.report.id}`, stored);
    return stored;
  }
...
async function request(
  ...
  const response = await fetch(`http://127.0.0.1:${address.port}${path}`, {
    ...
    headers: {
      ...
      ...(method === 'POST'
        ? { 'content-type': 'application/json', 'idempotency-key': 'specialised-app-mount' }
        : {}),
    },
...
describe('specialised API mounts', () => {
  ...
  it('routes property history through the main request handler rather than generic 404', async () => {
    ...
    const response = await request(
      deps(repository),
      '/api/v1/properties/missing-property/history',
      'GET',
    );
