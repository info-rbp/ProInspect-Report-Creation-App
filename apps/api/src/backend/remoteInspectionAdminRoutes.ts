...
async function readJson(req: IncomingMessage) { const chunks: Buffer[] = []; for await (const chunk of req) chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk)); if (!chunks.length) return {}; const parsed = JSON.parse(Buffer.concat(chunks).toString('utf8')) as unknown; if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new ApiError(400, 'INVALID_JSON', 'Request body must be a JSON object.'); return parsed as Record<string, unknown>; }
function webBaseUrl(): string { return process.env.WEB_APP_BASE_URL?.trim().replace(/\/$/u, '') || ''; }

export async function routeRemoteInspectionAdminRequest(req: IncomingMessage, deps: ApiDependencies, correlationId: string): Promise<ApiResponse | undefined> {
  const route = parts(req);
  if (route[0] !== 'api' || route[1] !== 'v1' || route[2] !== 'remote-inspection-assignments' || !route[3] || route[4] !== 'issue' || req.method !== 'POST') return undefined;
  ...
  const updated = await deps.repository.update('remoteInspectionAssignments', agency, assignment.id, { status: 'issued', issuedAt: new Date().toISOString(), issuedBy: principal.uid, portalGrantId: grantId, portalGrantExpiresAt: expiresAt }, Number(body.expectedVersion || assignment.version), principal.uid);
  return { status: 200, body: { data: { assignment: updated, accessUrl, expiresAt }, meta: { correlationId } } };
}
