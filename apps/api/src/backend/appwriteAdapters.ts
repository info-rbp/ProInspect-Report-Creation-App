...
const COLLECTION_TABLES: Readonly<Record<string, string>> = {
  ...
  buildings: 'buildings',
  propertyLevels: 'property_levels',
  propertyAreas: 'property_areas',
  serviceDefinitions: 'service_definitions',
  serviceRequests: 'service_requests',
  serviceRequestItems: 'service_request_items',
  shopifyServiceMappings: 'shopify_service_mappings',
  inspectionRequests: 'inspection_requests',
  inspectionJobs: 'inspection_jobs',
  inspectionTemplates: 'inspection_templates',
  inspectionTemplateVersions: 'inspection_template_versions',
  inspections: 'inspections',
  inspectionSections: 'inspection_sections',
  observations: 'observations',
  inspectionEvidence: 'inspection_evidence',
  reports: 'reports',
  reportVersions: 'report_versions',
  maintenanceCandidates: 'maintenance_candidates',
  maintenanceItems: 'maintenance_items',
  maintenanceQuotes: 'maintenance_quotes',
  maintenanceEstimates: 'maintenance_estimates',
  maintenanceApprovals: 'maintenance_approvals',
  maintenanceWorkOrders: 'maintenance_work_orders',
  maintenanceVariations: 'maintenance_variations',
  ...
  tenantInstructions: 'tenant_instructions',
  externalContacts: 'external_contacts',
  dailyActivityLogs: 'daily_activity_logs',
  residentRequests: 'resident_requests',
  contractorAttendance: 'contractor_attendance',
  keyTransactions: 'key_transactions',
  accessDevices: 'access_devices',
  accessDeviceHistory: 'access_device_history',
  moveBookings: 'move_bookings',
  residentOnboarding: 'resident_onboarding',
  operationalInspections: 'operational_inspections',
  assets: 'assets',
  maintenancePlans: 'maintenance_plans',
  wasteEvents: 'waste_events',
  incidents: 'incidents',
  bylawObservations: 'bylaw_observations',
  notices: 'notices',
  communications: 'communications',
  handovers: 'handovers',
  handoverChecklistItems: 'handover_checklist_items',
  operationalReportDrafts: 'operational_report_drafts',
  operationalReports: 'operational_reports',
  people: 'people',
  tenants: 'tenants',
  tenancies: 'tenancies',
  tenancyParticipants: 'tenancy_participants',
  occupancies: 'occupancies',
  units: 'units',
  contractors: 'contractors',
  propertyClientRelationships: 'property_client_relationships',
  keyRegister: 'key_register',
  accessDeviceRequests: 'access_device_requests',
  defects: 'defects',
  operationalInspectionCheckpoints: 'operational_inspection_checkpoints',
  operationalInspectionResults: 'operational_inspection_results',
  serviceEvents: 'service_events',
  wasteServices: 'waste_services',
  tasks: 'tasks',
  ...
  propertyOperatingSettings: 'property_operating_settings',
  operationalQuotes: 'operational_quotes',
  operationalApprovals: 'operational_approvals',
  operationalWorkOrders: 'operational_work_orders',
  reportReviewComments: 'report_review_comments',
  reportDistributions: 'report_distributions',
  reportAcknowledgements: 'report_acknowledgements',
  reportRecipientResponses: 'report_recipient_responses',
  reportSupersessions: 'report_supersessions',
  tenantCommunications: 'tenant_communications',
  tenancyDocuments: 'tenancy_documents',
  tenantPortalGrants: 'tenant_portal_grants',
  auditEvents: 'audit_events',
  integrationConnections: 'integration_connections',
  integrationEvents: 'integration_events',
  integrationDeliveries: 'integration_deliveries',
  integrationExceptions: 'integration_exceptions',
...
function publicRecord(row: Record<string, unknown>): StoredRecord {
  ...
  return {
    ...
    version: explicitVersion ?? timestampVersion,
    createdAt,
    updatedAt,
  } as StoredRecord;
}
...
export class AppwriteOperationalRepository implements OperationalRepository {
  ...
  async list(collection: string, agencyId: string, limit: number, cursor?: string): Promise<Page<StoredRecord>> {
    const queries = [Query.equal('agencyId', [agencyId]), Query.limit(limit), ...(cursor ? [Query.cursorAfter(cursor)] : [])];
    const result = await this.services.tables.listRows({
      databaseId: this.services.databaseId,
      tableId: tableId(collection),
      queries,
    });
  ...
  async create(collection: string, agencyId: string, id: string, data: Record<string, unknown>, actorId: string): Promise<StoredRecord> {
    const row = await this.services.tables.createRow({
      ...
      tableId: tableId(collection),
      rowId: id,
      data: writeData({ ...data, agencyId }, actorId),
      permissions: [],
    });
...
export class AppwriteMembershipRepository implements MembershipRepository {
  ...
  async getMembership(uid: string, agencyId: string): Promise<AgencyMembership | undefined> {
    const [agencyRows, siteRows] = await Promise.all([
    ...
    return {
      ...
      siteIds: siteRows.rows.map((item) => String((item as unknown as Record<string, unknown>).managedSiteId)).filter(Boolean),
  ...
  async append(event: SecurityAuditEvent): Promise<void> {
    ...
    await this.services.tables.createRow({
      ...
      data: {
        ...
        sourceCreatedAt: event.timestamp || now,
...
export function appwriteCollectionTableMap(): Readonly<Record<string, string>> {
