...
interface BuildingResourcePolicy {
  ...
  createCapability?: BuildingManagementCapability;
  siteScoped: boolean;
}
...
const RESOURCE_POLICIES: Readonly<Record<string, BuildingResourcePolicy>> = {
  ...
  'resident-requests': { collection: 'residentRequests', readCapability: 'resident.request.read', createCapability: 'resident.request.create', writeCapability: 'resident.request.manage', siteScoped: true },
  ...
  'access-device-requests': { collection: 'accessDeviceRequests', readCapability: 'access_device.read', createCapability: 'access_device.request', writeCapability: 'access_device.manage', siteScoped: true },
  ...
  'move-bookings': { collection: 'moveBookings', readCapability: 'move_booking.read', createCapability: 'move_booking.request', writeCapability: 'move_booking.manage', siteScoped: true },
  ...
  defects: { collection: 'defects', readCapability: 'defect.read', createCapability: 'defect.create', writeCapability: 'defect.manage', siteScoped: true },
  ...
  incidents: { collection: 'incidents', readCapability: 'incident.read', createCapability: 'incident.create', writeCapability: 'incident.manage', siteScoped: true },
  'bylaw-observations': { collection: 'bylawObservations', readCapability: 'bylaw.read', createCapability: 'bylaw.create', writeCapability: 'bylaw.decide', siteScoped: true },
...
function target(record: Record<string, unknown>, agencyId: string): AuthorisationTarget {
  return {
    ...
    ...(typeof record.submittedByUserId === 'string' ? { residentUserId: record.submittedByUserId } : {}),
...
function requireManagedSite(policy: BuildingResourcePolicy, data: Record<string, unknown>): void {
  if (policy.siteScoped && typeof data.managedSiteId !== 'string') throw new ApiError(400, 'MANAGED_SITE_REQUIRED', 'managedSiteId is required for this Building Management resource.');
}
...
async function appendAudit(dependencies: ApiDependencies, principal: { uid: string; role: string; agencyId: string }, capability: SecurityCapability, eventType: string, entityType: string, entityId: string, correlationId: string, metadata: Record<string, unknown>, eventTarget?: AuthorisationTarget): Promise<void> {
  await dependencies.audit.append({
    id: randomUUID(), timestamp: new Date().toISOString(), actorId: principal.uid, actorRole: principal.role,
    agencyId: principal.agencyId, capability, outcome: 'allowed', reason: eventType,
    target: eventTarget ?? { agencyId: principal.agencyId }, correlationId, eventType, entityType, entityId, metadata,
  });
}
...
async function transition(req: IncomingMessage, dependencies: ApiDependencies, correlationId: string, policy: BuildingResourcePolicy, resourceName: string, id: string, body: Record<string, unknown>): Promise<ApiResponse> {
  ...
  const resourceTarget = target(existing, agencyId);
  const principal = await authenticateAndAuthorise(req, dependencies, policy.writeCapability, resourceTarget, correlationId);
  ...
  if (resourceName === 'defects') {
    ...
    if (requested === 'closed') {
      const closure = canCloseDefect({ status: existing.status as DefectStatus, hasCompletionEvidence: body.hasCompletionEvidence === true, verifiedByUserId: typeof existing.verifiedBy === 'string' ? existing.verifiedBy : null });
    ...
    if (requested === 'closed') {
      const closure = canCloseMoveBooking({ status: existing.status as MoveBookingStatus, keysReturned: body.keysReturned === true, hasPostMoveInspection: body.hasPostMoveInspection === true });
  ...
  } else throw new ApiError(404, 'TRANSITION_NOT_SUPPORTED', 'Lifecycle transitions are not configured for this resource.');
  ...
  return idempotent(dependencies, req, agencyId, `building:${resourceName}:${id}:transition`, body, async () => {
    const updated = await dependencies.repository.update(policy.collection, agencyId, id, { status: requested, ...extraPatch, ...(typeof body.reason === 'string' ? { transitionReason: body.reason.trim() } : {}) }, version, principal.uid);
    await appendAudit(dependencies, principal, policy.writeCapability as SecurityCapability, `building.${resourceName}.transition`, resourceName, id, correlationId, { from: existing.status, to: requested }, resourceTarget);
    return { status: 200, body: { data: updated, meta: { correlationId } } };
  });
}

async function signOutAttendance(req: IncomingMessage, dependencies: ApiDependencies, correlationId: string, policy: BuildingResourcePolicy, id: string, body: Record<string, unknown>): Promise<ApiResponse> {
  const agencyId = agencyHeader(req);
  const existing = await dependencies.repository.get(policy.collection, agencyId, id);
  if (!existing) throw new ApiError(404, 'NOT_FOUND', 'Attendance record not found.');
  const resourceTarget = target(existing, agencyId);
  const principal = await authenticateAndAuthorise(req, dependencies, 'contractor.attendance.manage', resourceTarget, correlationId);
  const version = expectedVersion(body);
  const decision = canSignOutContractorAttendance({ keyIssued: body.keyIssued === true, keyReturned: body.keyReturned === true, overrideReason: typeof body.overrideReason === 'string' ? body.overrideReason : null });
  ...
  return idempotent(dependencies, req, agencyId, `building:contractor-attendance:${id}:sign-out`, body, async () => {
    ...
    await appendAudit(dependencies, principal, 'contractor.attendance.manage', 'building.contractor_attendance.sign_out', 'contractor_attendance', id, correlationId, { override: decision.requiresOverride }, resourceTarget);
    return { status: 200, body: { data: updated, meta: { correlationId } } };
  });
}

export async function routeBuildingManagementRequest(req: IncomingMessage, dependencies: ApiDependencies, correlationId: string): Promise<ApiResponse | undefined> {
  ...
  if (req.method === 'POST' && id && command === 'transitions') return transition(req, dependencies, correlationId, policy, resourceName, id, await readJson(req));
  if (req.method === 'POST' && resourceName === 'contractor-attendance' && id && command === 'sign-out') return signOutAttendance(req, dependencies, correlationId, policy, id, await readJson(req));
  ...
  if (req.method === 'GET') {
    ...
    const filters = managedSiteId ? { managedSiteId } : undefined;
    const page = await dependencies.repository.list(policy.collection, agencyId, limit, url.searchParams.get('cursor') ?? undefined, filters);
  ...
  if (req.method === 'POST' && !id) {
    const capability = policy.createCapability ?? policy.writeCapability;
    if (!capability) throw new ApiError(405, 'METHOD_NOT_ALLOWED', 'This resource is read-only.');
    const data = cleanWriteBody(body);
    requireManagedSite(policy, data);
    const principal = await authenticateAndAuthorise(req, dependencies, capability, target(data, agencyId), correlationId);
    if (['resident.request.create', 'move_booking.request', 'access_device.request'].includes(capability)) {
      data.submittedByUserId = principal.uid;
      data.residentUserId = principal.uid;
    }
    ...
    return idempotent(dependencies, req, agencyId, `building:${resourceName}:create`, body, async () => {
      ...
      await appendAudit(dependencies, principal, capability, `building.${resourceName}.created`, resourceName, recordId, correlationId, {}, target(data, agencyId));
      return { status: 201, body: { data: created, meta: { correlationId } } };
    });
  }

  if ((req.method === 'PATCH' || req.method === 'PUT') && id) {
    if (!policy.writeCapability) throw new ApiError(405, 'METHOD_NOT_ALLOWED', 'This resource is read-only and must be superseded through its dedicated command.');
    const existing = await dependencies.repository.get(policy.collection, agencyId, id);
    if (!existing) throw new ApiError(404, 'NOT_FOUND', 'Record not found.');
    ...
    return idempotent(dependencies, req, agencyId, `building:${resourceName}:${id}:update`, body, async () => {
      ...
      await appendAudit(dependencies, principal, policy.writeCapability as SecurityCapability, `building.${resourceName}.updated`, resourceName, id, correlationId, {}, resourceTarget);
      return { status: 200, body: { data: updated, meta: { correlationId } } };
    });
  }
