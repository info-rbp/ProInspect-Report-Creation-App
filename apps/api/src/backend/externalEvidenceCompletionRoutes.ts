...
export async function routeExternalEvidenceCompletionRequest(
  ...
  if (parts.length !== 8 || parts[0] !== 'api' || parts[1] !== 'v1' || parts[2] !== 'external' || parts[3] !== 'evidence' || !parts[4] || parts[5] !== 'upload-session' || !parts[6] || parts[7] !== 'complete') return undefined;
  ...
  if (!objectPath || !/^[a-f0-9]{64}$/u.test(expectedSha) || !Number.isFinite(expectedSize)) throw new ApiError(409, 'UPLOAD_SESSION_INVALID', 'Upload session is missing immutable provenance metadata.');
  ...
  try {
    ...
    throw new ApiError(409, 'EVIDENCE_OBJECT_NOT_FOUND', 'Uploaded evidence object is not available yet.');
  }
  ...
  await dependencies.repository.update('uploadSessions', grant.agencyId, uploadId, {
    status: 'uploaded', generation, sha256: actualSha, uploadedAt: now,
