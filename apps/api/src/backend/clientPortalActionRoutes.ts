...
async function conversations(req: IncomingMessage, deps: ApiDependencies, correlationId: string, clientAccountId: string, conversationId?: string, nested?: string): Promise<ApiResponse> {
  ...
  if (conversationId && nested === 'messages') {
    ...
    return idempotent(deps, req, agency, `client-portal:${clientAccountId}:conversation:${conversationId}:message`, body, async () => { const id = randomUUID(); const created = await deps.repository.create('conversationMessages', agency, id, { conversationId, senderType: 'user', senderId: principal.uid, channel: 'portal', body: message, attachmentFileIds: '[]', deliveryStatus: 'created', sentAt: new Date().toISOString(), status: 'active' }, principal.uid); await appendAudit(deps, principal, 'communication.send', 'client_portal.message.created', 'conversation_message', id, correlationId, target(agency, clientAccountId, typeof conversation.propertyId === 'string' ? conversation.propertyId : undefined)); return { status: 201, body: { data: created, meta: { correlationId } } }; });
  }
...
async function quoteDecision(req: IncomingMessage, deps: ApiDependencies, correlationId: string, clientAccountId: string, quoteId: string): Promise<ApiResponse> {
  ...
  const quote = await deps.repository.get('maintenanceQuotes', agency, quoteId); if (!quote) throw new ApiError(404, 'QUOTE_NOT_FOUND', 'Quote was not found.'); const itemId = String(quote.maintenanceItemId ?? ''); const item = itemId ? await deps.repository.get('maintenanceItems', agency, itemId) : undefined; if (!item || typeof item.propertyId !== 'string') throw new ApiError(409, 'QUOTE_SCOPE_UNRESOLVED', 'The quote is not linked to a client property.'); await assertProperty(deps, agency, clientAccountId, item.propertyId);
  ...
  return idempotent(deps, req, agency, `client-portal:${clientAccountId}:quote:${quoteId}:decision`, body, async () => {
    ...
    await appendAudit(deps, principal, 'communication.send', 'client_portal.quote.decision', 'maintenance_quote', quoteId, correlationId, target(agency, clientAccountId, item.propertyId));
