      ...
      ? [
          {
            ...
            in: 'path',
            required: true,
            schema: { type: 'string', enum: ['publish', 'duplicate', 'retire'] },
          },
        ]
      : []),
  ];
...
export function buildOpenApiDocument() {
  ...
  for (const resource of ['maintenance-items', 'work-requests', 'tenant-instructions'] as const) {
    paths[`/api/v1/${resource}/create`] = {
      post: commandOperation(
        resource,
        `create${resource.replace(/-([a-z])/gu, (_match, letter: string) => letter.toUpperCase())}`,
        `Creates a ${resource} record in its safe initial lifecycle state. Caller-supplied lifecycle status is ignored.`,
      ),
    };
    paths[`/api/v1/${resource}/{id}/actions/{action}`] = {
      post: {
        ...commandOperation(
          resource,
          `transition${resource.replace(/-([a-z])/gu, (_match, letter: string) => letter.toUpperCase())}`,
          'Executes a versioned, idempotent lifecycle business command.',
        ),
        parameters: [
          { name: 'x-agency-id', in: 'header', required: true, schema: { type: 'string' } },
          {
  ...
  paths['/api/v1/templates/drafts'] = {
    post: {
      ...
      tags: ['templates'],
      description:
        'Creates a new editable template business version in draft state. Published and retired states cannot be supplied.',
      security: [{ bearerAuth: [], appCheck: [], agency: [] }],
      parameters: [
        { name: 'x-agency-id', in: 'header', required: true, schema: { type: 'string' } },
        {
        ...
        content: {
          'application/json': {
            schema: {
              type: 'object',
              required: ['template'],
              properties: { template: { type: 'object', additionalProperties: true } },
            },
          },
        },
      },
  ...
  paths['/api/v1/templates/{id}/versions/{version}'] = {
    get: {
      ...
      responses: {
        '200': { description: 'Exact template version' },
    ...
    put: {
      ...
      tags: ['templates'],
      description:
        'Replaces an editable draft using expectedRecordVersion optimistic locking. Published and retired versions are immutable.',
      security: [{ bearerAuth: [], appCheck: [], agency: [] }],
      parameters: [
      ...
      requestBody: {
        ...
        content: {
          'application/json': {
            schema: {
              type: 'object',
              required: ['expectedRecordVersion', 'template'],
              properties: {
  ...
  paths['/api/v1/templates/{id}/versions/{version}/actions/{action}'] = {
    post: {
      ...
      tags: ['templates'],
      description:
        'Publishes, duplicates or retires an exact template version. Publishing makes the version immutable and updates the logical published pointer used for new reports.',
      security: [{ bearerAuth: [], appCheck: [], agency: [] }],
      parameters: [
        ...templateParameters(true, true),
        {
          name: 'Idempotency-Key',
          in: 'header',
          required: true,
      ...
      requestBody: {
        ...
        content: {
          'application/json': {
            schema: {
              type: 'object',
              required: ['expectedRecordVersion'],
              properties: { expectedRecordVersion: { type: 'integer', minimum: 1 } },
            },
          },
        },
      },
      responses: {
        '200': { description: 'Template lifecycle action completed' },
        '201': { description: 'New draft version created by duplicate' },
        '400': { $ref: '#/components/responses/Error' },
        '409': { $ref: '#/components/responses/Error' },
      },
      'x-required-capability': 'template.manage',
  ...
  paths['/api/v1/inspection-jobs/{id}/transitions'] = {
    post: {
      ...
      responses: {
        '200': { description: 'Workflow transition completed' },
        '409': { $ref: '#/components/responses/Error' },
      },
    },
  };
            ...
            schema: {
              ...
              properties: {
                ...
                allowLegacyBaseline: {
                  type: 'boolean',
                  description:
                    'Permits a clearly marked legacy_unstructured Entry placeholder only when no eligible immutable structured Entry exists.',
                },
                areas: {
                  type: 'array',
                  minItems: 1,
  ...
  paths['/api/v1/reports/{id}/legacy-baseline'] = {
    post: {
      ...
      description:
        'Saves a human-reviewed mapping from an older unstructured Entry source to stable Exit area/component IDs. Confidence is capped for legacy sources and operational status cannot be confirmed without explicit tested evidence.',
      requestBody: {
        required: true,
        content: {
          'application/json': {
            schema: {
              type: 'object',
              required: ['expectedVersion', 'source', 'mappings'],
              properties: {
      ...
      responses: {
        '200': { description: 'Legacy mapping updated' },
        '201': { description: 'Legacy mapping created' },
        '400': { $ref: '#/components/responses/Error' },
        '409': { $ref: '#/components/responses/Error' },
      },
  ...
  paths['/api/v1/maintenance-reports/create'] = {
    post: {
      ...
      description:
        'Creates or reuses a targeted Maintenance / Follow-Up report and linked inspection job for one or more Maintenance Items from the same property/tenancy context. Source facts become baseline only; the current assessment starts unconfirmed and operational components start untested.',
      security: [{ bearerAuth: [], appCheck: [], agency: [] }],
      parameters: [
        { name: 'x-agency-id', in: 'header', required: true, schema: { type: 'string' } },
        {
      ...
      responses: {
        '200': { description: 'Existing deterministic follow-up report reused' },
        '201': { description: 'Targeted follow-up report created' },
        '400': { $ref: '#/components/responses/Error' },
        '409': { $ref: '#/components/responses/Error' },
      },
  ...
  paths['/api/v1/properties/{id}/history'] = {
    get: {
      ...
      description:
        'Projects read-only property/component history from immutable report versions and linked Maintenance records. Historical report content is never mutated.',
      security: [{ bearerAuth: [], appCheck: [], agency: [] }],
      parameters: [
        { name: 'x-agency-id', in: 'header', required: true, schema: { type: 'string' } },
        { name: 'id', in: 'path', required: true, schema: { type: 'string' } },
      ...
      responses: {
        '200': { description: 'Immutable property/component history' },
        '404': { $ref: '#/components/responses/Error' },
      },
  ...
  paths['/api/v1/external/evidence/{grantToken}/upload-session'] = {
    post: {
      ...
      description:
        'Creates a resumable immutable evidence upload session scoped by a valid, unexpired Work Request or Tenant Instruction access grant. This does not yet create authoritative photoEvidence metadata.',
      security: [],
      parameters: [
      ...
      requestBody: {
        ...
        content: {
          'application/json': {
            schema: {
              type: 'object',
              required: ['fileName', 'contentType', 'size', 'sha256'],
              properties: {
      ...
      responses: {
        '201': { description: 'Scoped resumable evidence upload session' },
        '400': { $ref: '#/components/responses/Error' },
        '401': { $ref: '#/components/responses/Error' },
        '403': { $ref: '#/components/responses/Error' },
      },
  ...
  paths['/api/v1/external/evidence/{grantToken}/upload-session/{uploadId}/complete'] = {
    post: {
      ...
      tags: ['external-access-grants', 'uploads'],
      description:
        'Completes a grant-scoped evidence upload by reloading the exact upload session, verifying the stored object size and SHA-256, recording the exact Cloud Storage generation, and creating the canonical photoEvidence record idempotently.',
      security: [],
      parameters: [
        { name: 'grantToken', in: 'path', required: true, schema: { type: 'string', minLength: 16 } },
      ...
      responses: {
        '200': { description: 'Canonical evidence record already existed or was completed' },
        '201': { description: 'Canonical photoEvidence record created' },
        '400': { $ref: '#/components/responses/Error' },
        '401': { $ref: '#/components/responses/Error' },
        '403': { $ref: '#/components/responses/Error' },
        '409': { $ref: '#/components/responses/Error' },
  ...
  paths['/api/v1/reports/{id}/aggregate'] = {
    get: {
      ...
      responses: {
        '200': { description: 'Decomposed report metadata, areas and components' },
        '404': { $ref: '#/components/responses/Error' },
      },
    },
    put: {
      ...
      description:
        'Atomically creates or replaces editable report metadata, areas and components. Binary media is rejected.',
      responses: {
  ...
  paths['/api/v1/reports/{id}/transitions'] = {
    post: {
      ...
      description:
        'Atomically changes report and job state, assignment, audit, notification and immutable version records.',
      responses: {
        '200': { description: 'Report transition completed' },
        '409': { $ref: '#/components/responses/Error' },
      },
    },
  };
  ...
  return {
    ...
    info: {
      ...
      version: '1.2.0',
      description:
        'Server-authoritative Cloud Run API for agency-scoped property inspection operations.',
    },
