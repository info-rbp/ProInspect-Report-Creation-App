...
async function readJson(req: IncomingMessage): Promise<Record<string, unknown>> {
  ...
  try { const parsed = JSON.parse(Buffer.concat(chunks).toString('utf8')) as unknown; if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('object required'); return parsed as Record<string, unknown>; }
...
function expectedVersion(value: unknown): number | undefined {
  ...
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) throw new ApiError(400, 'EXPECTED_VERSION_INVALID', 'expectedVersion must be a positive integer.');
  return value;
}
...
export async function routeSettingsRequest(req: IncomingMessage, dependencies: ApiDependencies, correlationId: string): Promise<ApiResponse | undefined> {
  ...
  const suppliedVersion = expectedVersion(body.expectedVersion); const cleanBody = { ...body };
