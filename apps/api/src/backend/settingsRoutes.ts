...
import {
  agencyOperationalSettingsSchema,
...
const SECTION_CONFIG: Record<SettingsSection, {
  id: string;
  ...
  organisation: {
    ...
    writeCapability: 'settings.organisation.manage',
    schema: agencyOrganisationSettingsSchema as unknown as ValidationSchema<Record<string, unknown>>,
  },
  operational: {
    id: 'operational',
    readCapability: 'settings.read',
    writeCapability: 'settings.operations.manage',
    schema: agencyOperationalSettingsSchema as unknown as ValidationSchema<Record<string, unknown>>,
  },
  communications: {
    id: 'communications',
    readCapability: 'settings.read',
    writeCapability: 'settings.communications.manage',
    schema: communicationPolicySchema as unknown as ValidationSchema<Record<string, unknown>>,
  },
  maintenance: {
    id: 'maintenance',
    readCapability: 'settings.read',
    writeCapability: 'maintenance.policy.manage',
    schema: maintenancePolicySettingsSchema as unknown as ValidationSchema<Record<string, unknown>>,
  },
};
...
function expectedVersion(value: unknown): number | undefined {
  ...
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
    throw new ApiError(400, 'EXPECTED_VERSION_INVALID', 'expectedVersion must be a positive integer.');
  }
  return value;
}

export async function routeSettingsRequest(
  req: IncomingMessage,
  dependencies: ApiDependencies,
  correlationId: string,
): Promise<ApiResponse | undefined> {
  const route = parts(req);
  if (route[0] !== 'api' || route[1] !== 'v1' || route[2] !== 'settings') return undefined;
  ...
  if (!section || !(section in SECTION_CONFIG)) {
    if (route.length === 3 && req.method === 'GET') {
      ...
      const records = await dependencies.repository.list('agencySettings', agencyId, 20);
      return { status: 200, body: { data: records.items, meta: { actor: principal.uid, correlationId } } };
    }
    return undefined;
  }
  ...
  if (req.method === 'GET') {
    const principal = await authenticateAndAuthorise(req, dependencies, config.readCapability, { agencyId }, correlationId);
    const record = await dependencies.repository.get('agencySettings', agencyId, config.id);
    return { status: 200, body: { data: record ?? null, meta: { actor: principal.uid, correlationId } } };
  }

  if (req.method !== 'PUT' && req.method !== 'PATCH') {
    throw new ApiError(405, 'METHOD_NOT_ALLOWED', 'Settings sections support GET, PUT and PATCH only.');
  }

  const body = await readJson(req);
  const principal = await authenticateAndAuthorise(req, dependencies, config.writeCapability, { agencyId }, correlationId);
  const suppliedVersion = expectedVersion(body.expectedVersion);
  const cleanBody = { ...body };
  delete cleanBody.expectedVersion;
  delete cleanBody.id;
  delete cleanBody.agencyId;
  delete cleanBody.version;
  delete cleanBody.createdAt;
  delete cleanBody.updatedAt;
  const validated = validate(config.schema.parse(cleanBody));
  const current = await dependencies.repository.get('agencySettings', agencyId, config.id);
