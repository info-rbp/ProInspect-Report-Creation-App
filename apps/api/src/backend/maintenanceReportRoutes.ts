...
async function targetAreas(agencyId: string, items: Array<Versioned<MaintenanceItem>>): Promise<ReportAggregate['areas']> {
  ...
  const areaMap = new Map<string, ReportAggregate['areas'][number]>();
  items.forEach((item, index) => {
    ...
    const existing = areaMap.get(areaId) ?? { id: areaId, name: areaName, sequence: areaMap.size + 1, overallCommentary: '', photoReferences: [], components: [] };
