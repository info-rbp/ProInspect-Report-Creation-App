...
export async function routeLegacyBaselineRequest(
  ...
  if (!mappingRecord.propertyId) throw new ApiError(409, 'PROPERTY_REQUIRED', 'Exit report has no property context for legacy mapping.');
  ...
  await dependencies.audit.append({
    ...
    target: { agencyId, ...(aggregate.report.propertyId ? { propertyId: aggregate.report.propertyId } : {}), ...(aggregate.report.tenancyId ? { tenancyId: aggregate.report.tenancyId } : {}), reportId },
