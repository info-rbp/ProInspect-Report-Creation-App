...
function mediaType(value: unknown): EvidenceMediaType | undefined { return value === 'photo' || value === 'photo_360' || value === 'video' || value === 'audio' ? value : undefined; }
...
export class FirebaseUploadSessionIssuer implements UploadSessionIssuer {
  ...
  async create(agencyId: string, uploadId: string, input: Record<string, unknown>, principal: AuthenticatedPrincipal): Promise<Record<string, unknown>> {
    ...
    const externalResourceType = input.externalResourceType === 'work_request' || input.externalResourceType === 'tenant_instruction' || input.externalResourceType === 'report_distribution' || input.externalResourceType === 'tenant_portal' || input.externalResourceType === 'remote_inspection' ? input.externalResourceType : undefined;
    const session: UploadSessionRecord = { id: uploadId, agencyId, propertyId: String(input.propertyId), inspectionJobId, ...(typeof input.reportId === 'string' ? { reportId: input.reportId } : {}), ...(typeof input.areaId === 'string' ? { areaId: input.areaId } : {}), componentIds: Array.isArray(input.componentIds) ? input.componentIds.filter((value): value is string => typeof value === 'string') : [], originalFilename: fileName, contentType: String(input.contentType), fileSize: Number(input.size), sha256, objectPath, ...(mediaType(input.mediaType) ? { mediaType: mediaType(input.mediaType) } : {}), status: 'issued', issuedTo: principal.uid, expiresAt, ...(typeof input.externalGrantId === 'string' ? { externalGrantId: input.externalGrantId } : {}), ...(externalResourceType ? { externalResourceType } : {}), ...(typeof input.externalResourceId === 'string' ? { externalResourceId: input.externalResourceId } : {}), createdAt: now, updatedAt: now };
    ...
    const file = getStorage(adminApp()).bucket(this.bucketName).file(objectPath); const [resumableUploadUrl] = await file.createResumableUpload({ metadata: { contentType: session.contentType, metadata: { agencyId, inspectionJobId, uploadSessionId: uploadId, sha256, immutableOriginal: 'true', mediaType: session.mediaType || 'photo' } }, preconditionOpts: { ifGenerationMatch: 0 } }); session.resumableUploadUrl = resumableUploadUrl; await this.evidence.createSession(session); return session as unknown as Record<string, unknown>;
  }
}
