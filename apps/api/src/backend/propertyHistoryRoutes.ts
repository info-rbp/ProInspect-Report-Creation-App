...
function photoIds(value: unknown): string[] {
  return Array.isArray(value)
    ? value
      .map((item) => item && typeof item === 'object' && !Array.isArray(item) ? text((item as Record<string, unknown>).photoId) : '')
      .filter(Boolean)
    : [];
}
...
export async function routePropertyHistoryRequest(
  ...
  for (const report of reportRecords) {
    ...
    const immutableVersions = versions.docs
      ...
      .sort((left, right) => number(left.get('sequence')) - number(right.get('sequence')) || text(left.get('createdAt')).localeCompare(text(right.get('createdAt'))));
    ...
    inspections.push({
      ...
      templateVersion: number(report.templateVersion),
