...
function additionalData(agencyId: string, connectionId: string): string {
  return `${agencyId}:${connectionId}`;
}

export async function saveIntegrationCredentials<T>(
  ...
  const encryptedSecret = encryptIntegrationSecret(
    input.credentials,
    additionalData(input.agencyId, input.connectionId),
  );
...
export async function loadIntegrationCredentials<T>(
  dependencies: ApiDependencies,
  agencyId: string,
  connectionId: string,
): Promise<T | undefined> {
  ...
  return decryptIntegrationSecret<T>(
    credential.encryptedSecret,
    additionalData(agencyId, connectionId),
  );
}
