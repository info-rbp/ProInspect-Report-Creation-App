...
const PropertyLayoutEditor: React.FC<{
  ...
  return <div className="space-y-5">
    ...
    {!migration.complete && property.layoutTemplateId && <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
      <div className="flex gap-3"><AlertTriangle className="mt-0.5 shrink-0 text-amber-700" size={18} /><div className="flex-1"><h3 className="font-bold text-amber-950">Legacy layout requires canonical migration</h3><p className="mt-1 text-xs text-amber-800">The known template can migrate {migration.migratedCount} Area slots without using display-name guesses. {migration.unmappedCount} custom Area{migration.unmappedCount === 1 ? '' : 's'} will remain explicitly unmapped for review.</p><button disabled={busy || migration.migratedCount === 0} onClick={() => void saveRooms(migration.rooms, 'Migrated legacy template-backed layout to canonical Area and Component references', property.layoutTemplateId)} className="mt-3 rounded-xl bg-amber-900 px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">Create Canonical Layout Version</button></div></div>
    </section>}

    {migration.complete && migration.migratedCount > 0 && <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-xs text-blue-900">This legacy template can be upgraded deterministically to canonical references. Saving the migration creates a new Property Layout Version and does not rewrite historical Reports.</section>}
