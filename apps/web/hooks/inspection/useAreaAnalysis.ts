  ...
  const generateComponentCommentary = useCallback(
    async (areaName: string, item: InspectionItem, photos: Photo[], previousReportNotes?: string): Promise<string | null> => {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : 'Component analysis failed.';
        setAnalysisError(msg);
        return null;
      } finally {
  ...
  const generateAreaOverallCommentary = useCallback(
    async (areaName: string, _items: InspectionItem[], photos: Photo[]): Promise<string | null> => {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : 'Area commentary generation failed.';
        setAnalysisError(msg);
        return null;
      } finally {
  ...
  const discoverNewComponents = useCallback(
    async (areaName: string, photos: Photo[]): Promise<InspectionItem[]> => {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : 'Component discovery failed.';
        setAnalysisError(msg);
        return [];
      } finally {
