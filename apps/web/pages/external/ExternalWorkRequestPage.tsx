        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 border-t border-gray-100 pt-4">
            ...
            <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4">
              <div className="flex items-start gap-3"><ImagePlus className="mt-0.5 text-gray-500" size={18} /><div className="flex-1"><div className="text-xs font-semibold text-gray-800">Completion photographs</div><p className="mt-1 text-[11px] text-gray-500">Upload JPEG, PNG, HEIC or HEIF evidence. Images are stored as immutable originals and linked only to this scoped work request.</p><label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-lg bg-white px-3 py-2 text-xs font-medium text-gray-800 ring-1 ring-gray-300 hover:bg-gray-100"><UploadCloud size={15} />{uploading ? 'Uploading...' : 'Add Evidence'}<input className="hidden" type="file" accept="image/jpeg,image/png,image/heic,image/heif" multiple disabled={uploading} onChange={(event) => void handleEvidence(event)} /></label></div></div>
              {uploadedNames.length > 0 && <ul className="mt-3 space-y-1 text-[11px] text-emerald-700">{uploadedNames.map((name, index) => <li key={`${name}-${index}`}>Evidence uploaded: {name}</li>)}</ul>}
            </div>
