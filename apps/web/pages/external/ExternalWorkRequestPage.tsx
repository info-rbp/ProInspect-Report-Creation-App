        ...
        <div className="border-b border-gray-100 pb-4 flex items-center justify-between">
          <div>
            <div className="text-xs font-bold uppercase text-gray-400">
              ProInspect Contractor Portal
            </div>
            <h1 className="text-lg font-bold text-gray-900 mt-1">
              Work Request #{workRequest.id.slice(0, 8)}
            </h1>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 border-t border-gray-100 pt-4">
            <label className="block text-xs font-semibold text-gray-700">
              Contractor Notes & Progress Update
            </label>
            <textarea
              rows={3}
              value={notes}
            ...
            {canComplete && (
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                <div className="flex items-center gap-2 text-xs font-semibold text-gray-700">
                  <ImagePlus size={15} /> Completion evidence
                </div>
                <p className="mt-1 text-[11px] leading-4 text-gray-500">
                  Optional image evidence is uploaded through the scoped access link and verified
                  against its SHA-256 before it is attached to the completion response.
                </p>
                <input
                  type="file"
                  multiple
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <button
                ...
                className="rounded-lg border border-gray-300 bg-white py-2 text-xs font-medium text-gray-800 hover:bg-gray-100 disabled:opacity-50"
              >
                Acknowledge Request
              </button>
              <button
                ...
                className="rounded-lg bg-blue-600 py-2 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-50"
              >
                Mark In Progress
              </button>
              <button
                ...
                className="rounded-lg bg-emerald-600 py-2 text-xs font-medium text-white hover:bg-emerald-700 disabled:opacity-50"
              >
                {busy ? 'Submitting...' : 'Submit Completion'}
              </button>
              <button
                disabled={busy}
                onClick={() => void handleAction('unable_to_complete')}
                className="rounded-lg bg-gray-700 py-2 text-xs font-medium text-white hover:bg-gray-800 disabled:opacity-50"
              >
                Unable to Complete
              </button>
            </div>
