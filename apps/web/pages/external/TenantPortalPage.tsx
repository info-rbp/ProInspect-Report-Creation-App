...
const TenantPortalPage: React.FC = () => {
  ...
  return <div className="min-h-screen bg-slate-50">
    ...
    <main className="mx-auto max-w-5xl space-y-5 px-4 py-6 sm:px-6">
      ...
      {tab === 'documents' && <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-black">Documents & Forms</h2><div className="mt-4 space-y-3">{context.documents.length ? context.documents.map((item) => <div key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex justify-between"><div><div className="text-sm font-bold">{item.title}</div><div className="text-xs text-slate-500">{label(item.type)}</div></div><span className="text-[10px] font-bold uppercase">{label(item.status)}</span></div></div>) : <Empty text="No tenancy documents have been shared yet." />}</div><div className="mt-5 border-t border-slate-100 pt-5"><h3 className="text-sm font-black">Inspection Reports</h3><div className="mt-3 grid gap-3 md:grid-cols-2">{context.reports.map((item) => <div key={item.id} className="rounded-xl bg-slate-50 p-4"><div className="text-sm font-bold">{item.reportType}</div><div className="mt-1 text-xs text-slate-500">{date(item.inspectionDate)} · {label(item.lifecycleStatus)}</div></div>)}</div></div></section>}
