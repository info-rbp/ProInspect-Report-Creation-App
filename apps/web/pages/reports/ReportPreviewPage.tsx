    ...
    <div className="min-h-screen bg-gray-600 py-8 print:bg-white print:p-0 print:m-0 print:h-auto print:w-full">
      <div className="fixed top-4 right-4 flex max-w-[calc(100vw-2rem)] flex-wrap justify-end gap-3 no-print z-50">
        ...
        <button
          ...
          disabled={isQueueingPdf || !report.currentVersionId || Boolean(pdfJob)}
          title={!report.currentVersionId ? 'Reviewer approval must create an immutable report version first.' : undefined}
        ...
        {(pdfJob || pdfError) && (
          <div className={`w-full max-w-xl rounded-lg px-3 py-2 text-xs shadow-lg ${pdfError ? 'bg-rose-50 text-rose-800' : 'bg-emerald-50 text-emerald-800'}`}>
            {pdfError
              ? pdfError
