...
function formatAuthError(err: unknown): string {
  ...
  if (message.includes('auth/invalid-credential') || message.includes('auth/wrong-password') || message.includes('auth/user-not-found')) {
    return 'Invalid email or password. Use Google sign-in if your administrator provisioned that method.';
  }
...
const LoginRoutePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (mfaEnrollmentDetails || enrollmentLoading || enrollmentAttempted) return;
    setEnrollmentAttempted(true);
    void startEnrollment();
  }, [enrollmentAttempted, enrollmentLoading, mfaEnrollmentDetails, mfaState, startEnrollment]);
          ...
          <div className="rounded-xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-950 space-y-3">
            ...
            {mfaEnrollmentDetails && (
              <>
                ...
                <a
                  href={mfaEnrollmentDetails.qrCodeUrl}
                  className="inline-flex text-xs font-semibold text-indigo-800 underline underline-offset-2"
                >
                  Open authenticator setup link
                </a>
              </>
            )}
            {!enrollmentLoading && !mfaEnrollmentDetails && enrollmentAttempted && (
              <button
                type="button"
                onClick={() => void startEnrollment()}
                className="text-xs font-semibold text-indigo-800 underline underline-offset-2"
              >
                Retry MFA setup
              </button>
            )}
