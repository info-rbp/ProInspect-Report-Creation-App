...
const PriceBooksPanel: React.FC<{
  ...
  const chooseFile = async (file?: File) => {
    ...
    onBusy(true);
    try {
      const value = await preparePriceBookSpreadsheet(file);
      setPrepared(value);
      setName(file.name.replace(/\.[^.]+$/u, ''));
    } catch (failure) {
      onError(failure instanceof Error ? failure.message : 'Spreadsheet could not be prepared.');
    } finally {
      onBusy(false);
    }
  };
  ...
  const stage = async () => {
    ...
    onBusy(true);
    try {
      await uploadPriceBookSpreadsheet(prepared, name.trim());
      setPrepared(null);
      await onChanged();
    } catch (failure) {
      onError(failure instanceof Error ? failure.message : 'Price-book upload failed.');
    } finally {
      onBusy(false);
    }
  };
  ...
  const publish = async (record: PriceBookImport) => {
    onBusy(true);
    try {
      await publishPriceBookImport(record, {
        name: record.proposedPriceBookName || name,
        currency,
        effectiveFrom,
      });
      await onChanged();
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-500">
          The original XLSX, XLS or CSV is uploaded as an immutable, SHA-256 verified source before its rows enter staging.
        </p>
        <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_220px_120px_150px_auto]">
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 px-4 py-3 text-xs font-semibold text-slate-600 hover:bg-slate-50">
            <Upload size={15} /> {prepared ? prepared.fileName : 'Choose spreadsheet'}
            <input
              type="file"
              accept=".xlsx,.xls,.csv"
              className="hidden"
        ...
        {prepared && (
          <div className="mt-4 grid gap-3 rounded-xl bg-slate-50 p-4 text-xs sm:grid-cols-4">
            <div><b>Worksheet</b><br />{prepared.sheetName}</div>
            <div><b>Rows</b><br />{prepared.rows.length}</div>
            <div><b>File size</b><br />{Math.round(prepared.fileSize / 1024).toLocaleString()} KB</div>
            <div><b>SHA-256</b><br /><span className="break-all font-mono text-[10px]">{prepared.sha256.slice(0, 24)}…</span></div>
          </div>
        )}
      </section>
        ...
        <div className="mt-4 space-y-3">
            ...
            <div key={item.id} className="rounded-xl border border-slate-200 p-4">
              <div className="flex flex-col justify-between gap-3 lg:flex-row">
                <div>
                  ...
                  <div className="mt-2 grid gap-3 text-xs text-slate-500 sm:grid-cols-5">
                    ...
                    <div>{item.source.generation ? 'Source verified' : 'Source metadata only'}</div>
                  </div>
          ...
          {!imports.length && (
            <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">
              No pricing imports have been staged.
            </div>
          )}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          ...
          {!books.length && (
            <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">
              No price book has been published.
            </div>
          )}
        </div>
      </section>
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <button
          ...
          onClick={() => void create()}
          className="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-xs font-semibold text-white disabled:opacity-50"
        >
      ...
      {!policies.length && (
        <div className="rounded-2xl border border-dashed border-slate-300 p-12 text-center text-sm text-slate-500">
          No approval policies are configured. The server uses a conservative owner-approval default.
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-500">Use this where ProInspect owns one dedicated Xero organisation and has Custom Connection credentials.</p>
