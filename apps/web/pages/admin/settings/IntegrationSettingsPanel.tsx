...
const IntegrationSettingsPanel: React.FC = () => {
  ...
  const refresh = async (provider: string) => {
    ...
    try {
      if (provider === 'shopify') {
        await apiRequest(userProfile.agencyId, '/api/v1/integrations/shopify/reconcile', { method: 'POST', body: {} });
      } else if (provider === 'google_calendar') {
        await apiRequest(userProfile.agencyId, '/api/v1/integrations/google-calendar/reconcile', { method: 'POST', body: {} });
      } else {
        await apiRequest(userProfile.agencyId, '/api/v1/integrations/xero/status');
      }
      await load();
