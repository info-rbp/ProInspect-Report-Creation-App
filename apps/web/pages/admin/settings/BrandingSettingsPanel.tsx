    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <label className="text-xs font-semibold">Existing profile<select className={field} value={selectedId} onChange={(event) => setSelectedId(event.target.value)}><option value="">Create new profile</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name} · {profile.status}</option>)}</select></label>
