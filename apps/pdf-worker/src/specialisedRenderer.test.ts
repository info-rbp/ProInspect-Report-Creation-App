...
import { buildRenderPackage } from './index.js';
...
describe('specialised PDF presentation', () => {
  it('renders Routine reports exception-first while retaining the same structured input', async () => {
    const routineInput = input('Routine Inspection');
    const entryInput = input('Property Condition Report');
    expect(routineInput.areas).toEqual(entryInput.areas);
    const routinePages = await pages(routineInput);
    const entryPages = await pages(entryInput);
