...
function tenantEvidenceIds(response: Record<string, unknown>): string[] {
  const direct = [response.evidenceIds, response.photoEvidenceIds, response.tenantEvidenceIds]
    .flatMap((value) => asArray(value))
    .map(valueText)
    .filter(Boolean);
  const components = asArray(response.componentResponses)
    .flatMap((value) => asArray(asRecord(value).evidenceIds))
    .map(valueText)
    .filter(Boolean);
...
export async function renderReportPdf(input: RenderInput, imageBytes: ReadonlyMap<string, Uint8Array> = new Map()): Promise<Uint8Array> {
  ...
  if (input.tenantResponses?.length) {
    ...
    for (const response of input.tenantResponses) {
      ...
      for (const rawComponentResponse of asArray(response.componentResponses)) {
        ...
        const agrees = typeof componentResponse.agrees === 'boolean' ? `Tenant agrees: ${componentResponse.agrees ? 'Yes' : 'No'}` : '';
