...
import type { ApiDependencies, OperationalRepository, Page, ReportAggregateStore, ReportTransitionCommand, StoredRecord } from '../../apps/api/src/backend/types.js';
...
class MemoryRepository implements OperationalRepository {
  ...
  async create(collection: string, agencyId: string, id: string, data: Record<string, unknown>, actorId: string) { const now = new Date().toISOString(); const record = { ...data, id, agencyId, version: 1, createdAt: now, updatedAt: now, createdBy: actorId } as StoredRecord; this.values.set(this.key(collection, agencyId, id), record); return record; }
  async update(collection: string, agencyId: string, id: string, data: Record<string, unknown>, expectedVersion: number, actorId: string) { const existing = await this.get(collection, agencyId, id); if (!existing) throw new Error('not found'); if (existing.version !== expectedVersion) throw new Error('version conflict'); const record = { ...existing, ...data, version: expectedVersion + 1, updatedAt: new Date().toISOString(), updatedBy: actorId } as StoredRecord; this.values.set(this.key(collection, agencyId, id), record); return record; }
}
...
async function request(deps: ApiDependencies, token: string, uploadId: string) {
  ...
  const response = await fetch(`http://127.0.0.1:${address.port}/api/v1/external/evidence/${encodeURIComponent(token)}/upload-session/${encodeURIComponent(uploadId)}/complete`, { method: 'POST' });
...
describe('external evidence provenance', () => {
  it('creates canonical photoEvidence only after verifying exact object bytes', async () => {
    ...
    await db.doc('agencies/agency-a/externalAccessGrants/grant-1').set({
      id: 'grant-1', agencyId: 'agency-a', resourceType: 'work_request', resourceId: 'work-1', recipientEmail: 'contractor@example.test', tokenHash, expiresAt: '2099-01-01T00:00:00.000Z', version: 1,
    });

    const bytes = Buffer.from('canonical external evidence bytes');
    const sha256 = createHash('sha256').update(bytes).digest('hex');
    const uploadId = 'upload-1';
    const objectPath = 'agencies/agency-a/properties/property-1/jobs/job-1/evidence/upload-1/original/photo.jpg';
    await getStorage().bucket('demo-pcr-uploads').file(objectPath).save(bytes, { resumable: false, contentType: 'image/jpeg' });

    const repository = new MemoryRepository();
    await repository.create('uploadSessions', 'agency-a', uploadId, {
      objectPath, sha256, size: bytes.length, contentType: 'image/jpeg', propertyId: 'property-1', inspectionJobId: 'job-1', componentIds: ['component-1'], externalGrantId: 'grant-1', externalResourceType: 'work_request', externalResourceId: 'work-1', status: 'issued',
    }, 'external:grant-1');
    ...
    expect(evidence.data()).toMatchObject({ id: uploadId, agencyId: 'agency-a', propertyId: 'property-1', inspectionJobId: 'job-1', objectPath, sha256, status: 'uploaded', source: 'external_portal', externalGrantId: 'grant-1' });
    expect(String(evidence.get('generation') ?? '')).not.toBe('');
    expect(await repository.get('uploadSessions', 'agency-a', uploadId)).toMatchObject({ status: 'uploaded', sha256 });
  });
  ...
  it('rejects an expired grant before canonical evidence is created', async () => {
    ...
    await db.doc('agencies/agency-a/externalAccessGrants/grant-expired').set({
      id: 'grant-expired', agencyId: 'agency-a', resourceType: 'tenant_instruction', resourceId: 'instruction-1', recipientEmail: 'tenant@example.test', tokenHash, expiresAt: '2020-01-01T00:00:00.000Z', version: 1,
    });
